...
class Car_EweiShopV2ComModel extends ComModel 
{
	...
	{
		global $_W;
		$car = pdo_fetch('SELECT id,total FROM ' . tablename('ewei_shop_car') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		if (empty($car)) 
		{
			return 0;
		}
		...
		$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_car_data') . ' where couponid=:couponid and uniacid=:uniacid ', array(':couponid' => $couponid, ':uniacid' => $_W['uniacid']));
		return $car['total'] - $gettotal;
	}
	public function creditshop($logid = 0) 
	{
		...
		if (!(empty($log))) 
		{
			...
			$car = pdo_fetch('select * from ' . tablename('ewei_shop_car') . ' where id=:id limit 1', array(':id' => $log['couponid']));
			$car = $this->setCoupon($car, time());
			$this->sendMessage($car, 1, $member);
	...
	public function payResult($logno) 
	{
		...
		{
			$update = array();
			if ((0 < $car['credit']) && empty($log['creditstatus'])) 
			{
				m('member')->setCredit($log['openid'], 'credit1', -$car['credit'], '购买优惠券扣除积分 ' . $car['credit']);
				$update['creditstatus'] = 1;
			}
			if ((0 < $car['money']) && empty($log['paystatus'])) 
			{
				if ($log['paytype'] == 0) 
				{
					m('member')->setCredit($log['openid'], 'credit2', -$car['money'], '购买优惠券扣除余额 ' . $car['money']);
				}
				$update['paystatus'] = 1;
			}
			$update['status'] = 1;
		...
		if ($car['coupontype'] == 0) 
		{
			$car['url'] = mobileUrl('goods', NULL, true);
		}
		else 
		{
			$car['url'] = mobileUrl('member/recharge', NULL, true);
		}
		return $car;
	}
	public function sendMessage($car, $send_total, $member, $account = NULL) 
	{
		global $_W;
		$articles = array();
		$title = str_replace('[nickname]', $member['nickname'], $car['resptitle']);
		$desc = str_replace('[nickname]', $member['nickname'], $car['respdesc']);
		$title = str_replace('[total]', $send_total, $title);
		$desc = str_replace('[total]', $send_total, $desc);
		$url = ((empty($car['respurl']) ? mobileUrl('sale/car/my', NULL, true) : $car['respurl']));
		if (!(empty($car['resptitle']))) 
		{
			$articles[] = array('title' => urlencode($title), 'description' => urlencode($desc), 'url' => $url, 'picurl' => tomedia($car['respthumb']));
		}
		if (!(empty($articles))) 
		{
	...
	public function sendBackMessage($openid, $car, $gives) 
	{
		global $_W;
		if (empty($gives)) 
		{
		...
		$content = '您的优惠券【' . $car['couponname'] . '】已返利 ';
		$givestr = '';
	...
	public function sendReturnMessage($openid, $car) 
	{
		global $_W;
		$set = m('common')->getPluginset('car');
		$templateid = $set['templateid'];
		$msg = array( 'keyword1' => array('value' => '优惠券退回', 'color' => '#73a68d'), 'keyword2' => array('value' => '您的优惠券【' . $car['couponname'] . '】已退回您的账户，您可以再次使用, 谢谢!', 'color' => '#73a68d') );
		$url = mobileUrl('sale/car/my', NULL, true);
		if (!(empty($templateid))) 
		{
			m('message')->sendTplNotice($openid, $templateid, $msg, $url);
		}
	...
	public function useRechargeCoupon($log) 
	{
		...
		$car = pdo_fetch('select enough,backcredit,backmoney,backredpack,couponname from ' . tablename('ewei_shop_car') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
		if (empty($car)) 
		{
			return;
		}
		if ((0 < $car['enough']) && ($log['money'] < $car['enough'])) 
		{
			return;
		}
		$gives = array();
		$backcredit = $car['backcredit'];
		if (!(empty($backcredit))) 
		{
		...
		$this->sendBackMessage($log['openid'], $car, $gives);
	}
	...
	public function useConsumeCoupon($orderid = 0) 
	{
		...
		{
			$car = $this->getCouponByDataID($order['couponid']);
		}
		if (empty($car)) 
		{
			return;
	...
	public function backConsumeCoupon($orderid) 
	{
		...
		$gives = array();
		$canback = false;
		if (($order['status'] == 1) && ($car['backwhen'] == 2)) 
		{
			$canback = true;
		}
		else 
		{
			...
			if ($is_done == 1) 
			{
				...
				else if ($car['backwhen'] == 1) 
				{
					$canback = true;
				}
				else if ($car['backwhen'] == 0) 
				{
					$canback = true;
		...
		if ($canback) 
		{
			...
			$this->sendBackMessage($order['openid'], $car, $gives);
		}
	}
	public function getCoupon($couponid = 0) 
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('ewei_shop_car') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
	}
	...
	public function setCoupon($row, $time, $withOpenid = true) 
	{
		...
		if ($withOpenid) 
		{
			...
			if (0 < $row['getmax']) 
			{
				$gets = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_car_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $row['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
	...
	public function setShare() 
	{
		...
		$set = m('common')->getPluginset('car');
		$openid = $_W['openid'];
		$url = mobileUrl('sale/car', NULL, true);
	...
	public function poster($member, $couponid, $couponnum) 
	{
		...
		$set = m('common')->getPluginset('car');
		$this->sendMessage($car, $couponnum, $member);
	}
	public function posterbylist($list, $openid, $gettype) 
	{
		...
		{
			$url = $this->getUrl('sale/car/my/showcoupons3', array('key' => $showkey), true);
		}
		else 
		{
			$url = $this->getUrl('sale/car/my/showcoupons', array('key' => $showkey), true);
		}
	...
	public function getCoupons() 
	{
		global $_W;
		$time = time();
		return pdo_fetchall('select * from ' . tablename('ewei_shop_car') . ' where  (timelimit = 0  or  (c.timelimit =1 and c.timestart<={$time} && timeend>={$time})) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	}
}
?>