...
class Sms_EweiShopV2ComModel extends ComModel 
{
	public function send($mobile, $tplid, $data, $replace = true) 
	{
		...
		if ($template['type'] == 'ztinfo')
		{
					include_once EWEI_SHOPV2_VENDOR . 'ztinfo/php_Nsms_demo.php';
			$url 		= "http://www.ztsms.cn/sendNSms.do";//提交地址
			$username 	= $smsset['ztinfo_name'];//用户名
			$password 	= $smsset['ztinfo_pawd'];//原密码
			$sendAPI = new sendAPI($url, $username, $password);
			$code=implode('',$params);
			$data = array(
				'content' 	=> '您的验证码是'.$code.'。如非本人操作，请忽略本短信。',//短信内容
				'mobile' 	=> $mobile,//手机号码
				'productid' => $template['smstplid'],//产品id
				'xh'		=> ''//小号
			);
			$sendAPI->data = $data;//初始化数据包
			$result = $sendAPI->sendSMS('POST');//GET or POST

			if (empty($result) || !empty($result['0']))
			{
				if($result['0']!=1){
					return array('status' => 0, 'message' => '短信发送失败(返回码'.$result['0'].')');
				}
	...
	public function sms_temp() 
	{
		...
		foreach ($list as $i => &$item ) 
		{
			...
			else if ($item['type'] == 'ztinfo')
			{
				$item['name'] = '[助通]' . $item['name'];
			}
	...
	protected function sms_verify($tplid, $smsset) 
	{
		...
		else if ($template['type'] == 'ztinfo')
		{
			if (empty($smsset['ztinfo']))
			{
				return array('status' => 0, 'message' => '未开启助通短信!');
			}
			if (empty($smsset['ztinfo_name']))
			{
				return array('status' => 0, 'message' => '未填写助通短信Username!');
			}
			if (empty($smsset['ztinfo_pawd']))
			{
				return array('status' => 0, 'message' => '未填写助通短信password!');
			}
			if (empty($template['data']) || !is_array($template['data']))
			{
				return array('status' => 0, 'message' => '模板类型错误!');
