...
class Member_EweiShopV2Model 
{
	...
	public function getCredits($openid = '', $credittypes = array('credit1', 'credit2','credit3'))
	{
		global $_W;
		load()->model('mc');
		$uid = mc_openid2uid($openid);
	...
	public function checkMember() 
	{
		...
		if ($followed || empty($shopset['shop']['getinfo']) || ($shopset['shop']['getinfo'] == 1)) 
		{
			...
			$mc = array();
			if(strstr($userinfo['headimgurl'],'132132')){
				$userinfo['headimgurl'] = rtrim($userinfo['headimgurl'], '132') . 132;
			}else{
		...
		return array('id' => $member['id'], 'openid' => $member['openid'], 'realname' => $member['realname'], 'mobile' => $member['mobile']);
	}
	...
	//二级分销业务提成（分佣）
	public function order_commission($orderid)
	{
		global $_W;
		if (empty($orderid))
		{
		...
		$money1=$_W['shopset']['shop']['level_1'];//一级得到业务提成
		$money2=$_W['shopset']['shop']['level_2'];//一级得到业务提成
		$money3=$_W['shopset']['shop']['merch_merch'];//“商家”直推的会员升级商家可以得到推广费用
		$merch_level_2=$_W['shopset']['shop']['merch_level_2'];//“商家”可享受伞下二级内会员的
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		if (empty($item) || empty($item['goodspcate']))
		{
			return null;
		}
		if($item['goodspcate']!=1174){
			return null;
		}
		$member = m('member')->getMember($item['openid']);//订单购买者
		if (empty($member) || empty($item['agentid']))
		{
			return null;
		}
		...
		m('member')->setCredit($agent1['openid'], 'credit2',$money1, array($agent1['uid'], '一级业务提成'.$money1.'元,订单号:'.$item['ordersn']));
		if (!empty($agent1['agentid']) )
		{
			...
			m('member')->setCredit($agent2['openid'], 'credit2',$money2, array($agent2['uid'], '二级业务提成'.$money2.'元,订单号:'.$item['ordersn']));
			if($agent2['level']==1&&$agent1['level']==0){
				$money3=round($_W['shopset']['shop']['merch_level_1']*$money1/100,2);//“商家”可享受直推会员业务提成的40%佣金分红
				m('member')->setCredit($agent2['openid'], 'credit2',$money3, array($agent2['uid'], '“商家”可享受直推会员业务提成的'.$_W['shopset']['shop']['merch_level_1'].'%佣金分红'.$money3.'元,订单号:'.$item['ordersn']));
				$money4=round($_W['shopset']['shop']['merch_level_2']*$money1/100,2);//“商家”可享受伞下二级内会员的的5%的业绩分红
				m('member')->setCredit($agent2['openid'], 'credit2',$money4, array($agent2['uid'], '“商家”可享受伞下二级内会员的'.$_W['shopset']['shop']['merch_level_2'].'%的业绩分红'.$money4.'元,订单号:'.$item['ordersn']));
			}
			if (!empty($agent2['agentid']) ) {
				...
				if($agent3['level']==1&&$agent2['level']==0){
					$money5=round($_W['shopset']['shop']['merch_level_2']*$money2/100,2);//“商家”可享受伞下一级级内会员的的5%的业绩分红
					m('member')->setCredit($agent2['openid'], 'credit2',$money5, array($agent2['uid'], '“商家”可享受伞下一级会员的'.$_W['shopset']['shop']['merch_level_2'].'%的业绩分红'.$money5.'元,订单号:'.$item['ordersn']));
				}
				if($agent3['level']==1&&$agent1['level']==0){
					$money6=round($_W['shopset']['shop']['merch_level_2']*$money1/100,2);//“商家”可享受伞下二级内会员的的5%的业绩分红
					m('member')->setCredit($agent2['openid'], 'credit2',$money6, array($agent2['uid'], '“商家”可享受伞下二级会员的'.$_W['shopset']['shop']['merch_level_2'].'%的业绩分红'.$money6.'元,订单号:'.$item['ordersn']));
				}
				if (!empty($agent3['agentid']) ) {
					$agent4 = m('member')->getMember($agent3['agentid']);
					if (empty($agent4)) {
						return null;
					}
					if($agent4['level']==1&&$agent2['level']==0){
						$money7=round($_W['shopset']['shop']['merch_level_2']*$money2/100,2);//“商家”可享受伞下一级级内会员的的5%的业绩分红
						m('member')->setCredit($agent2['openid'], 'credit2',$money7, array($agent2['uid'], '“商家”可享受伞下二级会员的'.$_W['shopset']['shop']['merch_level_2'].'%的业绩分红'.$money7.'元,订单号:'.$item['ordersn']));
					}
				}
			}
	...
	//会员升级
	public function upmemberLevel($openid)
	{
		global $_W;
		if (empty($openid))
		{
		...
		if ($leveltype == 2)
		{
			$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid=:agentid  and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
			$level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . ' where uniacid=:uniacid and enabled=1 and ' . $ordercount . ' >= ordercount and ordercount>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
		}
		if (empty($level))
		{
		...
		if ($canupgrade)
		{
			...
			pdo_update('ewei_shop_member', array('level' => $level['id'],'islevel'=>1), array('id' => $member['id']));
			m('notice')->sendMemberUpgradeMessage($openid, $oldlevel, $level);
			if($member['islevel']==0){
				$money=$_W['shopset']['shop']['fanhui'];
				m('member')->setCredit($member['openid'], 'credit2',$money, array($member['uid'], '升级成商家奖励'.$money.'元'));
				$remark = '升级成商家奖励';
				$set = m('common')->getSysset('shop');
				$logno = m('common')->createNO('member_log', 'logno', 'RC');
				$data = array('openid' => $member['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' =>  '升级成商家奖励', 'money' => $money, 'remark' => $remark, 'rechargetype' => 'system2');
				pdo_insert('ewei_shop_member_log', $data);
				$logid = pdo_insertid();
				m('notice')->sendMemberLogMessage($logid, 0, true);
				//商家”直推的会员升级商家可以得到推广费$money1
				$money1=$_W['shopset']['shop']['merch_merch'];
				if(!empty($member['agentid'])){
					$aengt = m('member')->getMember($member['agentid']);
					if(!empty($aengt)){
						if(!empty($aengt['level'])&&$money1){
							m('member')->setCredit($aengt['openid'], 'credit3',$money1, array($aengt['uid'], '推广费'.$money1.'元'));
							$remark = '"商家"直推的会员升级商家可以得到推广费';
							$data = array('openid' => $aengt['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' =>  '推广费', 'money' => $money1, 'remark' => $remark, 'rechargetype' => 'system2');
							pdo_insert('ewei_shop_member_log', $data);
							$logid = pdo_insertid();
							m('notice')->sendMemberLogMessage($logid, 0, true);
						}
						//“商家”可享受伞下二层内会员的5%的业绩分红
						$bili=$_W['shopset']['shop']['merch_level_2'];
						$money2=round($bili*$money1/100,2);
						if(!empty($aengt['agentid'])){//下一级
							$aengt1 = m('member')->getMember($aengt['agentid']);
							if(!empty($aengt1)){
								if(!empty($aengt1['level'])&&$money2){
									m('member')->setCredit($aengt1['openid'], 'credit3',$money2, array($aengt1['uid'], '下一级业绩分红'.$money2.'元'));
									$remark = '“商家”可享受伞下一级会员推广费的'.$bili.'%的业绩分红';
									$data = array('openid' => $aengt['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' =>  '业绩分红', 'money' => $money2, 'remark' => $remark, 'rechargetype' => 'system2');
									pdo_insert('ewei_shop_member_log', $data);
									$logid = pdo_insertid();
									m('notice')->sendMemberLogMessage($logid, 5, true);
								}
								if(!empty($aengt1['agentid'])){//下二级
									$aengt2 = m('member')->getMember($aengt1['agentid']);
									if(!empty($aengt2)){
										if(!empty($aengt2['level'])&&$money2){
											m('member')->setCredit($aengt2['openid'], 'credit3',$money2, array($aengt2['uid'], '下二级业绩分红'.$money2.'元'));
											$remark = '“商家”可享受伞下二级会员推广费的'.$bili.'%的业绩分红';
											$data = array('openid' => $aengt2['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' =>  '业绩分红', 'money' => $money2, 'remark' => $remark, 'rechargetype' => 'system2');
											pdo_insert('ewei_shop_member_log', $data);
											$logid = pdo_insertid();
											m('notice')->sendMemberLogMessage($logid, 5, true);
										}
