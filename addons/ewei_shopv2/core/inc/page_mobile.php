...
class MobilePage extends Page
{
	...
	public function __construct()
	{
		...
		if(strpos($r,'account')=== false){
			if (empty($me['mobile'])) {
			if($member && strpos($r,'member.bind')=== false){

					header('location: ' . mobileUrl('member/bind'));
				}
			}elseif (empty($me['realname'])) {
			if($member && strpos($r,'member.info')=== false){
					...
					if (!(empty($diyitems)) && is_array($diyitems))
					{
						...
						if (strexists($jsondiyitems, 'fixedsearch'))
						{
							foreach ($diyitems as $diyitemid => $diyitem )
							{
								...
								//@lk 2017.11.15
                                /*foreach ($diyitem['data'] as $key => $goods ){
								    if(!empty($goods['gid'])){
                                        $good=pdo_fetch('select shorttitle,shoufu,yuegong,qishu,marketprice from ' . tablename('ewei_shop_goods') .' where uniacid = ' .$_W['uniacid']  .' and id ='.  $goods['gid']);
                                        $goods['shorttitle']=$good['shorttitle'];
                                        $goods['yuegong']=$good['yuegong'];
                                        $goods['qishu']=$good['qishu'];
                                        $goods['shoufu']=$good['shoufu'];
                                        $goods['price']=111;
                                    }
							...
							}//file_put_contents(__DIR__.'/a1.php',json_encode($diyitem));
							unset($diyitem);
						}
						unset($diyitem);

					}
                    //file_put_contents(__DIR__.'/a.php',json_encode($diyitems));
