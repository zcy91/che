...
class Withdraw_EweiShopV2Page extends MobileLoginPage
{
	...
	public function submit()
	{
		...
		$credit = m('member')->getCredit($_W['openid'], 'credit3');  //余额
		if ($money <= 0)
		{
			show_json(0, '提现金额错误!');
		}
		...
		$beishu = $_W['shopset']['trade']['withdrawmoney']; //最小提现额度
		$a = floor(($money/$beishu))*$beishu;
		if($money !== $a){
			show_json(0, '提现金额为'.$beishu.'倍数');
		}
		$apply = array();
		$type_array = array();
		if ($set['withdrawcashweixin'] == 1)
		...
		m('member')->setCredit($_W['openid'], 'credit3', -$money, array(0, $_W['shopset']['set'][''] . '佣金提现预扣除: ' . $money . ',实际到账金额:' . $realmoney . ',手续费金额:' . $deductionmoney));
