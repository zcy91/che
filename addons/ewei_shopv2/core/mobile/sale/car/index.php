...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		$sql = 'select * from ' . tablename('ewei_shop_car_category') . ' where uniacid=:uniacid';
		if ($is_openmerch == 0) 
		{
			$sql .= ' and merchid=0';
		}
	...
	public function getlist() 
	{
		...
		$param = array();
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select id,timelimit,coupontype,timedays,timestart,timeend,thumb,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor from ' . tablename('ewei_shop_car') . ' c ';
		...
		{
			$row = com('car')->setCoupon($row, $time);
			$last = com('car')->get_last_count($row['id']);
			if ($row['t'] != -1) 
			{
				if ($last <= 0) 
				{
		...
		unset($row);
		file_put_contents(__dir__."/coupons803.txt",json_encode($coupons));
