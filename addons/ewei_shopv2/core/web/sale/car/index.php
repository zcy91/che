...
class Index_EweiShopV2Page extends ComWebPage 
{
	...
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('ewei_shop_car') . ' ' . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		foreach ($list as &$row ) 
		{
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_car_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_car_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_car') . ' where 1 and ' . $condition, $params);
		$pager = pagination($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('ewei_shop_car_category') . ' where uniacid=:uniacid and merchid=0 order by id desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				{
					$pwdkey2 = pdo_fetch('SELECT pwdkey2 FROM ' . tablename('ewei_shop_car') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
					if ($pwdkey2['pwdkey2'] != $data['pwdkey2']) 
					{
						...
						if (!(empty($keyword))) 
						{
							show_json(0, array('url' => webUrl('sale/car/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab']))), 'message' => '口令关键词已存在!'));
						}
					}
				}
				pdo_update('ewei_shop_car', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				plog('sale.car.edit', '编辑洗车券 ID: ' . $id . ' <br/>洗车券名称: ' . $data['couponname']);
			}
			else 
			{
				if (!(empty($data['pwdkey2']))) 
				...
				$data2 = pdo_insert('ewei_shop_car', $data);
				$id = pdo_insertid();
				plog('sale.car.add', '添加洗车券 ID: ' . $id . '  <br/>洗车券名称: ' . $data['couponname']);
			}
			$key = 'ewei_shopv2:com:car:' . $id;
			...
			show_json(1, array('url' => webUrl('sale/car/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_car') . ' WHERE id =:id and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($item)) 
		{
			$starttime = time();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('ewei_shop_car') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('ewei_shop_car', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('ewei_shop_car_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid']));
			plog('sale.car.delete', '删除洗车券 ID: ' . $id . '  <br/>洗车券名称: ' . $item['couponname'] . ' ');
		}
		show_json(1, array('url' => webUrl('sale/car')));
	}
	public function displayorder() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('ewei_shop_car', array('displayorder' => $displayorder), array('id' => $id));
			plog('sale.car.displayorder', '修改洗车券排序 ID: ' . $item['id'] . ' 名称: ' . $item['couponname'] . ' 排序: ' . $displayorder . ' ');
		}
		show_json(1);
	}
	public function query() 
	{
		...
		$time = time();
		$ds = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_car') . '  WHERE 1 ' . $condition . ' ORDER BY id asc', $params);
		if (!(empty($ds))) 
		{
			foreach ($ds as &$d ) 
			{
				$d = com('car')->setCoupon($d, $time, false);
				$d['last'] = com('car')->get_last_count($d['id']);
				if ($d['last'] == -1) 
				{
					$d['last'] = '不限';
				}
	...
	public function querycoupons() 
	{
		...
		$ds = pdo_fetchall('SELECT id,couponname as title , thumb FROM ' . tablename('ewei_shop_car') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
	...
	public function send() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('ewei_shop_car', $upgrade, array('id' => $car['id']));
			...
			foreach ($members as $m ) 
			{
				...
				while ($i <= $send_total) 
				{
					$log = array('uniacid' => $_W['uniacid'], 'merchid' => $car['merchid'], 'openid' => $m['openid'], 'logno' => m('common')->createNO('coupon_log', 'logno', 'CC'), 'couponid' => $couponid, 'status' => 1, 'paystatus' => -1, 'creditstatus' => -1, 'createtime' => $time, 'getfrom' => 0);
					pdo_insert('ewei_shop_car_log', $log);
					$logid = pdo_insertid();
					$data = array('uniacid' => $_W['uniacid'], 'merchid' => $car['merchid'], 'backtype' => $car['backtype'], 'openid' => $m['openid'], 'couponid' => $couponid, 'gettype' => 0, 'gettime' => $time, 'senduid' => $_W['uid']);
					pdo_insert('ewei_shop_car_data', $data);
					++$i;
				}
				com('car')->sendMessage($car, $send_total, $m, $account);
			}
			plog('sale.car.send', $plog);
			show_json(1, array('message' => '洗车券发放成功!', 'url' => webUrl('sale/car')));
		}
	...
	public function set() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			plog('sale.car.set', '修改基本设置');
			show_json(1, array('url' => webUrl('sale/car/set', array('tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
	...
	public function update(){
		...
		if (!(empty($item))) 
		{
			pdo_update('ewei_shop_car_data',array('used'=>1,'usetime'=>time()),array('id'=>$id));
		}
		header("Location:https://jyqc.hejiewl.com/web/index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=sale.car.log");
	}
}
?>