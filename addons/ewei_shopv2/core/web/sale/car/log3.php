...
class Log3_EweiShopV2Page extends ComWebPage 
{
	...
	public function main() 
	{
		...
		/*if (!(empty($_GPC['time1']['start'])) && !(empty($_GPC['time1']['end']))) 
		{
			...
			$condition .= ' AND k.cardtime >= :starttime1 AND k.cardtime <= :endtime1 ';
		...
		$sql = 'SELECT k.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile,u.merchname FROM ' . tablename('ewei_shop_car_kadata') . ' k ' . ' left join '. tablename('ewei_shop_merch_user') . ' u on k.merchid = u.id ' . ' left join ' . tablename('ewei_shop_car') . ' c on k.couponid = c.id ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid = k.openid and m.uniacid = k.uniacid ' . ' where  1 and ' . $condition . ' ORDER BY usetime DESC';

		file_put_contents(__dir__."/sql001.txt",json_encode($sql));
