...
class Sendcoupon_EweiShopV2Page extends ComWebPage 
{
	...
	public function fetch() 
	{
		...
		$class1 = $_GPC['send1'];
		$coupon = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		//file_put_contents(__dir__."/725coupon.txt",json_encode($coupon));
		if (empty($coupon)) 
		{
			show_json(0, '未找到优惠券!');
		}
		...
		//file_put_contents(__dir__."/725members.txt",json_encode($members));
		if (empty($members)) 
		{
			show_json(0, '未找到发送的会员!');
		}
		...
		$time = time();
		$i = 1;
		$log = array('uniacid' => $_W['uniacid'], 'merchid' => $coupon['merchid'], 'openid' => $members[0]['openid'], 'logno' => m('common')->createNO('coupon_log', 'logno', 'CC'), 'couponid' => $couponid, 'status' => 1, 'paystatus' => -1, 'creditstatus' => -1, 'createtime' => $time, 'getfrom' => 0);
		pdo_insert('ewei_shop_coupon_log', $log);
		$logid = pdo_insertid();
		$data = array('uniacid' => $_W['uniacid'], 'merchid' => $coupon['merchid'], 'openid' => $members[0]['openid'], 'couponid' => $couponid, 'gettype' => 0, 'gettime' => $time, 'senduid' => $_W['uid']);
