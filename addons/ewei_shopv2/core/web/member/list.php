...
class List_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		if ($_GPC['leadership'] != '') {
			$condition .= ' and leadership=' . intval($_GPC['leadership']);
		}
	...
	public function detail()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($data['level']))
			{
				m('member')->upagentpath($member['openid']);
			}
			...
			if ($hascommission) {
				if (cv('commission.agent.edit')) {
					...
					if (!empty($adata)) {
						...
						if(!(empty($adata['agentid']))){
							$me = m('member')->getMember($adata['agentid']);
							$adata['agentpath']=$me['agentpath'].$adata['agentid'].',';
						}else{
							$adata['agentpath']=',0,';
						}
