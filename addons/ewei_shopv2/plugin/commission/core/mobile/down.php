...
class Down_EweiShopV2Page extends CommissionMobileLoginPage
{
	...
	'pagesize' => $psize
		...
		foreach ($list as &$row ) {
            ...
            $ordercountcar = pdo_fetchcolumn('select count(id) from ' . tablename('ewei_shop_order') . ' where openid=:openid and uniacid=:uniacid and status>=3 and goodspcate=1174 ', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
            $ordercountnum = pdo_fetchcolumn('select sum(zhidaojia) from ' . tablename('ewei_shop_order') . ' where openid=:openid and status>=3 and goodspcate=1174 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));

            $row['ordercountcar']=number_format(intval($ordercountcar), 0);
            $row['ordercountnum']=number_format(intval($ordercountnum), 0);
