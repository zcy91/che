...
if (!(class_exists('CommissionModel'))) 
{
	class CommissionModel extends PluginModel 
	{
				...
				if ($member['id'] != $parent['id']) 
				{
					if (empty($become_child)) 
					{
                        if(empty($parent['id'])){
                            ...
                            $agentpath = pdo_getcolumn('ewei_shop_member', array('id' => $parent['id']), 'agentpath', 1);
                            $agentpath = $agentpath . $parent['id'] . ',';
                        }
						if (empty($member['fixagentid'])) 
						{
							...
							if ($author) 
							{
								...
								pdo_update('ewei_shop_member', array('agentid' => $parent['id'], 'agentpath' => $agentpath, 'childtime' => $time, 'authorid' => $authorid), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
							else 
							{
								pdo_update('ewei_shop_member', array('agentid' => $parent['id'], 'agentpath' => $agentpath, 'childtime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
							if ($author) 
							{
		...
		public function checkOrderPay($orderid = '0') 
		{
            ...
            $update = array('paytime1' => $time, 'status1' => 1,'status2' => 1,'status3' => 1);
            pdo_update('ewei_shop_order_goods', $update, array('orderid' => $order['id']));
			if ($isagent) 
			{
		...
		public function checkOrderFinish($orderid = '') 
		{
			...
			//2017.9.29 新增判断购买汽车则升级成为会员
            /*if ($member['agentlevel'] == 0){
                $new_order_goods = pdo_fetchall('select goodsid from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid  ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']), 'goodsid');
                foreach($new_order_goods as $row){
                    $pcate = pdo_getcolumn('ewei_shop_goods', array('id' => $row['goodsid']), 'pcate', 1);
                    if ($pcate == 1174){
                        pdo_update('ewei_shop_member', array('agentlevel' => 3), array('id' => $member['id']));
                        break;
                    }
                }
			}*/
            ...
            //$member1 = $this ->getInfo($openid, array('total', 'ok', 'apply', 'check', 'lock', 'pay', 'wait', 'fail'));
			//@lk 2017.11.7
            $agentid1=$order['agentid'];
			$level11=$set['level'];
           if (!(empty($agentid1)))
            {
				$set = m('common')->getSysset('shop');
				$logno = m('common')->createNO('member_log', 'logno', 'RC');
                $m1 = m('member')->getMember($agentid1);
                $member1 = $this ->getInfo($m1['openid'], array('apply'));
                if (($m1['isagent'] == 1) && ($m1['status'] == 1))
                {

                    if($level11>=1 && !empty($member1['commission_apply'])){
                        m('member')->setCredit($m1['openid'], 'credit3', $member1['commission_apply'], array($m1['openid'],  '业务提成'.$member1['commission_apply']));
                        $remark = '一级业务提成';
                        $data = array('openid' => $m1['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '业务提成', 'money' => $member1['commission_apply'], 'remark' => $remark, 'rechargetype' => 'system2');
                        pdo_insert('ewei_shop_member_log', $data);
                        $logid = pdo_insertid();
                        m('notice')->sendMemberLogMessage($logid, 3, true);
                    }
                    if (!(empty($m1['agentid'])))
                    {
                        $m2 = m('member')->getMember($m1['agentid']);
                        $member2 = $this ->getInfo($m2['openid'], array('apply'));

                        if($level11>=2&&!empty($member2['commission_apply'])){
                            m('member')->setCredit($m2['openid'], 'credit3', $member2['commission_apply'], array($m2['openid'],  '业务提成'.$member2['commission_apply']));
							$remark = '二级业务提成';
                            $data = array('openid' => $m2['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '业务提成', 'money' => $member2['commission_apply'], 'remark' => $remark, 'rechargetype' => 'system2');
                            pdo_insert('ewei_shop_member_log', $data);
                            $logid = pdo_insertid();
                            m('notice')->sendMemberLogMessage($logid, 3, true);
                        }
						...
						$bili=$_W['shopset']['shop']['merch_level_1'];
						$money2=round($bili*$member1['commission_apply']/100,2);
						$bili1=$_W['shopset']['shop']['merch_level_2'];
						$money1=round($bili1*$member1['commission_apply']/100,2);
						if(!empty($member2['level'])&&$money2) {
							//“商家”可享受直推会员业务提成的40%佣金分红
							m('member')->setCredit($member2['openid'], 'credit3', $money2, array($member2['uid'], '业务提成的佣金分红' . $money2 . '元'));
							$remark = '“商家”可享受直推会员业务提成的' . $bili . '%的佣金分红';
							$data = array('openid' => $member2['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '佣金分红', 'money' => $money2, 'remark' => $remark, 'rechargetype' => 'system2');
							pdo_insert('ewei_shop_member_log', $data);
							$logid = pdo_insertid();
							m('notice')->sendMemberLogMessage($logid, 4, true);
						}
						if(!empty($member2['level'])&&$money1) {
							//“商家”可享受伞下一级内会员的'.$bili.'%的业绩分红'
							m('member')->setCredit($member2['openid'], 'credit3',$money1, array($member2['uid'], '下一级业务提成的业绩分红'.$money1.'元'));
							$remark = '“商家”可享受伞下一级会员业务提成的'.$bili1.'%的业绩分红';
							$data = array('openid' => $member2['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' =>  '业绩分红', 'money' => $money1, 'remark' => $remark, 'rechargetype' => 'system2');
							pdo_insert('ewei_shop_member_log', $data);
							$logid = pdo_insertid();
							m('notice')->sendMemberLogMessage($logid, 5, true);
						}
                        if (!(empty($m2['agentid']))) {
							$m3 = m('member')->getMember($m2['agentid']);
							$member3 = $this->getInfo($m3['openid'], array('apply'));

							if ($level11 >= 3 && !empty($member3['commission_apply'])) {
								m('member')->setCredit($m3['openid'], 'credit3', $member3['commission_apply'], array($m3['openid'], '业务提成' . $member3['commission_apply']));
								$remark = '三级业务提成';
								$set = m('common')->getSysset('shop');
								$logno = m('common')->createNO('member_log', 'logno', 'RC');
								$data = array('openid' => $m3['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '业务提成', 'money' => $member3['commission_apply'], 'remark' => $remark, 'rechargetype' => 'system2');
								pdo_insert('ewei_shop_member_log', $data);
								$logid = pdo_insertid();
								m('notice')->sendMemberLogMessage($logid, 3, true);
							}
							//@lk
							//“商家”可享受伞下二层内会员的5%的业绩分红
							$money3 = round($bili1 * $member2['commission_apply'] / 100, 2);
							$money4 = round($bili1 * $member1['commission_apply'] / 100, 2);
							$money6 = round($bili1 * $money2 / 100, 2);
							if (!empty($member3['level']) && $money6) {
								//“商家”可享受伞下一级内会员佣金分红的'.$bili.'%的业绩分红'
								m('member')->setCredit($member3['openid'], 'credit3', $money6, array($member3['uid'], '下一级佣金分红的业绩分红' . $money6 . '元'));
								$remark = '“商家”可享受伞下一级会员佣金分红的' . $bili1 . '%的业绩分红';
								$data = array('openid' => $member3['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '业绩分红', 'money' => $money6, 'remark' => $remark, 'rechargetype' => 'system2');
								pdo_insert('ewei_shop_member_log', $data);
								$logid = pdo_insertid();
								m('notice')->sendMemberLogMessage($logid, 5, true);
							}
							if (!empty($member3['level']) && $money3) {
								//“商家”可享受伞下一级内会员的'.$bili.'%的业绩分红'
								m('member')->setCredit($member3['openid'], 'credit3', $money3, array($member3['uid'], '下一级业务提成的业绩分红' . $money3 . '元'));
								$remark = '“商家”可享受伞下一级会员业务提成的' . $bili1 . '%的业绩分红';
								$data = array('openid' => $member3['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '业绩分红', 'money' => $money3, 'remark' => $remark, 'rechargetype' => 'system2');
								pdo_insert('ewei_shop_member_log', $data);
								$logid = pdo_insertid();
								m('notice')->sendMemberLogMessage($logid, 5, true);
							}
							if (!empty($member3['level']) && $money4) {
								//“商家”可享受伞下一级内会员的'.$bili.'%的业绩分红'
								m('member')->setCredit($member3['openid'], 'credit3', $money4, array($member3['uid'], '下二级业务提成的业绩分红' . $money4 . '元'));
								$remark = '“商家”可享受伞下二级会员业务提成的' . $bili1 . '%的业绩分红';
								$data = array('openid' => $member3['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '业绩分红', 'money' => $money4, 'remark' => $remark, 'rechargetype' => 'system2');
								pdo_insert('ewei_shop_member_log', $data);
								$logid = pdo_insertid();
								m('notice')->sendMemberLogMessage($logid, 5, true);
							}
							$money5 = round($bili1 * $member2['commission_apply'] / 100, 2);
							if (!(empty($m3['agentid']))) {
								$m4 = m('member')->getMember($m3['agentid']);
								$member4 = $this->getInfo($m4['openid'], array('apply'));
								if (!empty($member4['level']) && $money5) {
									//“商家”可享受伞下一级内会员的'.$bili.'%的业绩分红'
									m('member')->setCredit($member4['openid'], 'credit3', $money6, array($member4['uid'], '下二级佣金分红的业绩分红' . $money6 . '元'));
									$remark = '“商家”可享受伞下二级会员佣金分红的' . $bili1 . '%的业绩分红';
									$data = array('openid' => $member4['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '业绩分红', 'money' => $money6, 'remark' => $remark, 'rechargetype' => 'system2');
									pdo_insert('ewei_shop_member_log', $data);
									$logid = pdo_insertid();
									m('notice')->sendMemberLogMessage($logid, 5, true);
								}
								if (!empty($member4['level']) && $money5) {
									//“商家”可享受伞下一级内会员的'.$bili.'%的业绩分红'
									m('member')->setCredit($member4['openid'], 'credit3', $money5, array($member4['uid'], '下二级业务提成的业绩分红' . $money5 . '元'));
									$remark = '“商家”可享受伞下二级会员业务提成的' . $bili1 . '%的业绩分红';
									$data = array('openid' => $member4['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '业绩分红', 'money' => $money5, 'remark' => $remark, 'rechargetype' => 'system2');
									pdo_insert('ewei_shop_member_log', $data);
									$logid = pdo_insertid();
									m('notice')->sendMemberLogMessage($logid, 5, true);
								}
			...
			$update = array('paytime1' => $time, 'status1' => 3,'status2' => 3,'status3' => 3);
            pdo_update('ewei_shop_order_goods', $update, array('orderid' => $order['id']));
			if ($isagent) 
			{
