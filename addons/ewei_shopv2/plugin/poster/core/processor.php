			...
			if (!(empty($cset))) 
			{
				if ($member['isagent'] != 1) 
				{
					if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) 
					{
						...
						{
						    if(empty($qrmember['id'])){
						        $agentpath = ',0,';
                            }else{
                                $agentpath = pdo_getcolumn('ewei_shop_member', array('id' => $qrmember['id']), 'agentpath', 1);
                                $agentpath = $agentpath . $qrmember['id'] . ',';
                            }
							if (empty($member['agentid'])) 
							{
								if (empty($member['fixagentid'])) 
								{
									...
									if ($author) 
									{
										...
										pdo_update('ewei_shop_member', array('agentid' => $qrmember['id'], 'agentpath' => $agentpath, 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
									}
									else 
									{
										pdo_update('ewei_shop_member', array('agentid' => $qrmember['id'], 'agentpath' => $agentpath, 'childtime' => $time), array('id' => $member['id']));
									}
									if ($author) 
									{
