...
class List_EweiShopV2Page extends MerchWebPage 
{
	...
	protected function post() 
	{
		...
		global $_GPC;
		$merchid = $_W['merchid'];
		$item = $this->model->getMerchPrice($merchid, 1); //815 提现
		$list = $this->model->getMerchPriceList($merchid); //815 提现
		...
		file_put_contents(__dir__."/list815.txt",json_encode($list));
		
		$xianxiamoney = pdo_fetch('select * from ' . tablename('ewei_shop_merch_user') . ' where uniacid=:uniacid and id=' . $merchid, array(':uniacid' => $_W['uniacid']));
		
		if ($_W['ispost']) 
		{
			/*if (($item['realprice'] <= 0) || empty($list) || $xianxiamoney['xianxiamoney'] <= 0) */
			if (($item['realprice'] <= 0)) 
			{
			...
			$insert['orderprice'] = $item['orderprice']; //订单金额
			...
			pdo_update('ewei_shop_merch_user',array('xianxiamoney'=>0),array('id'=>$merchid));
