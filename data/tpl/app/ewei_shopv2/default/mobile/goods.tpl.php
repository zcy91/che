...
<script type='text/html' id='tpl_goods_list'>
	...
	<div class="detail">
	   <a <%if g.bargain>0%>href="<?php  echo mobileUrl('bargain/detail')?>&id=<%g.bargain%>"<%else%>href="<?php  echo mobileUrl('goods/detail')?>&id=<%g.id%>"<%/if%>>
	           <div class="name" style="height: 3rem;"><%g.title%><br /><%g.shorttitle%><br /><%if g.pcate==1174%>指导价:<%g.zhidaojia%>万元<%/if%></div>

	   </a>
	           <div class="price">
			   <%if g.pcate==1174%><span class="text">首付<%g.shoufu%>万&nbsp;月供<%g.yuegong%>元</span>
			   <%else%><span class="text"><%if g.merchid%>积分：<%else%>￥<%/if%><%g.minprice%></span>
			   <%/if%>
           <span class="buy" <%if g.pcate==1174%>style="display: none"<%/if%>><%if g.bargain >0%>砍价活动<%else%><i class="icon icon-cart"></i><%/if%></span></div>
 	    </div>
	</div>
    <%/each%>
</script>
