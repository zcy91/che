	...
	<span class='pull-right'>
		  <?php if(mcv('sale.coupon.add')) { ?>
					 <a class='btn btn-primary btn-sm' href="<?php  echo merchUrl('sale/coupon/add')?>"><i class='fa fa-plus'></i> 添加购物优惠券</a>
				       <?php  } ?>
		<a class="btn btn-default  btn-sm" href="<?php  echo merchUrl('sale/coupon')?>">返回列表</a>
	</span>
	<h2><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>购物优惠券 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['couponname'];?>】<?php  } ?></small></h2>
</div>
