...
<div class="page-toolbar row m-b-sm m-t-sm">
			  ...
			  <div class="input-group-btn">
				...
				<?php if(mcv('sale.coupon.delete')) { ?>	
			        <button class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php  echo merchUrl('sale/coupon/delete')?>"><i class='fa fa-trash'></i> 删除</button>
					<?php  } ?>
				  ...
				  <label class='label label-success'>购物</label>
					...
					<td>
						...
						<a href="javascript:void(0);" class="btn btn-default btn-sm" data-toggle="popover" data-trigger="hover" data-html="true"
						   data-content="<img src='<?php  echo $row['qrcode'];?>' alt='链接二维码'>">
							<i class="glyphicon glyphicon-qrcode"></i>
						</a>
						
                         <?php if(mcv('sale.coupon.edit')) { ?> 
                              <a class='btn btn-default btn-sm' href="<?php  echo merchUrl('sale/coupon/edit',array('id' => $row['id']));?>"><i class='fa fa-edit'></i> <?php if(mcv('sale.coupon.edit')) { ?>编辑<?php  } else { ?>查看<?php  } ?></a>
                        <?php  } ?>
                        <?php if(mcv('sale.coupon.delete')) { ?> 
                              <a class='btn btn-default  btn-sm' data-toggle='ajaxRemove' href="<?php  echo merchUrl('sale/coupon/delete',array('id' => $row['id']));?>" data-confirm="确定要删除该优惠券吗？"><i class='fa fa-trash'></i> 删除</a>
                        <?php  } ?>
                       </div>
