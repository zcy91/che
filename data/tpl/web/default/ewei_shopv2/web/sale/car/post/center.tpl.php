...
<div class="gettype" <?php  if($item['gettype']!=1) { ?>style="display:none"<?php  } ?>>
    ...
    <div class="form-group islimitlevel" <?php  if($item['islimitlevel']!=1) { ?>style="display:none"<?php  } ?>>
        ...
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('sale.coupon' ,$item) ) { ?>
            <label class="checkbox-inline"><input type="checkbox"  class="checkall" name="limitmemberlevels[]" value="0" <?php  if(in_array("0",$limitmemberlevels)) { ?> checked="true"  <?php  } ?>  /> <?php echo empty($shop['levelname'])?'普通等级':$shop['levelname']?></label>
            <?php  if(is_array($levels)) { foreach($levels as $level) { ?>
            <label class="checkbox-inline"><input type="checkbox"  class="checkall" name="limitmemberlevels[]" value="<?php  echo $level['id'];?>"  <?php  if(in_array( $level['id'] , $limitmemberlevels)) { ?> checked="true"  <?php  } ?>  /> <?php  echo $level['levelname'];?></label>
            <?php  } } ?>
            <?php  } else { ?>
            <?php  if(in_array("0",$limitmemberlevels)) { ?> <?php echo  empty($shop['levelname'])?'普通等级':$shop['levelname']?>  <?php  } ?>&nbsp;&nbsp;
            <?php  if(is_array($levels)) { foreach($levels as $level) { ?>
            <?php  if(in_array($level['id'] , $limitmemberlevels)) { ?>
        ...
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('sale.coupon' ,$item) ) { ?>
            <label class="checkbox-inline"><input type="checkbox"  class="checkall" name="limitagentlevels[]" value="0" <?php  if(in_array("0",$limitagentlevels)) { ?> checked="true"  <?php  } ?>  /> <?php echo empty($plugin_com_set['levelname'])?'普通等级':$plugin_com_set['levelname']?></label>
            <?php  if(is_array($agentlevels)) { foreach($agentlevels as $level) { ?>
            <label class="checkbox-inline"><input type="checkbox"  class="checkall" name="limitagentlevels[]" value="<?php  echo $level['id'];?>"  <?php  if(in_array( $level['id'] , $limitagentlevels)) { ?> checked="true"  <?php  } ?>  /> <?php  echo $level['levelname'];?></label>
            <?php  } } ?>
            <?php  } else { ?>
            <?php  if(in_array("0",$limitagentlevels)) { ?> <?php echo empty($plugin_com_set['levelname'])?'普通等级':$plugin_com_set['levelname']?>  <?php  } ?>&nbsp;&nbsp;
            <?php  if(is_array($agentlevels)) { foreach($agentlevels as $level) { ?>
            <?php  if(in_array( $level['id'] , $limitagentlevels)) { ?>
            <?php  echo $level['levelname']?>&nbsp;&nbsp;
            <?php  } ?>
            <?php  } } ?>
            <?php  } ?>
        ...
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('sale.coupon' ,$item) ) { ?>
                <label class="checkbox-inline"><input type="checkbox"  class="checkall" name="limitpartnerlevels[]" value="0" <?php  if(in_array("0",$limitpartnerlevels)) { ?> checked="true"  <?php  } ?>  /> <?php echo empty($plugin_globonus_set['levelname'])?'普通等级':$plugin_globonus_set['levelname']?></label>
                <?php  if(is_array($partnerlevels)) { foreach($partnerlevels as $level) { ?>
                    <label class="checkbox-inline"><input type="checkbox"  class="checkall" name="limitpartnerlevels[]" value="<?php  echo $level['id'];?>"  <?php  if(in_array( $level['id'] , $limitpartnerlevels)) { ?> checked="true"  <?php  } ?>  /> <?php  echo $level['levelname'];?></label>
                <?php  } } ?>
            <?php  } else { ?>
                <?php  if(in_array("0",$limitpartnerlevels)) { ?> <?php echo empty($plugin_globonus_set['levelname'])?'普通等级':$plugin_globonus_set['levelname']?>  <?php  } ?>&nbsp;&nbsp;
                <?php  if(is_array($partnerlevels)) { foreach($partnerlevels as $level) { ?>
                    <?php  if(in_array( $level['id'] , $limitpartnerlevels)) { ?>
                        <?php  echo $level['levelname']?>&nbsp;&nbsp;
                    <?php  } ?>
                <?php  } } ?>
            <?php  } ?>
        ...
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('sale.coupon' ,$item) ) { ?>
                <label class="checkbox-inline"><input type="checkbox"  class="checkall"  name="limitaagentlevels[]" value="0" <?php  if(in_array("0",$limitaagentlevels)) { ?> checked="true"  <?php  } ?>  /> <?php echo empty($plugin_abonus_set['levelname'])?'普通等级':$plugin_abonus_set['levelname']?></label>
                <?php  if(is_array($aagentlevels)) { foreach($aagentlevels as $level) { ?>
                    <label class="checkbox-inline"><input type="checkbox"   class="checkall" name="limitaagentlevels[]" value="<?php  echo $level['id'];?>"  <?php  if(in_array( $level['id'] , $limitaagentlevels)) { ?> checked="true"  <?php  } ?>  /> <?php  echo $level['levelname'];?></label>
                <?php  } } ?>
            <?php  } else { ?>
            <?php  if(in_array("0",$limitaagentlevels)) { ?> <?php echo empty($plugin_abonus_set['levelname'])?'普通等级':$plugin_abonus_set['levelname']?>  <?php  } ?>&nbsp;&nbsp;
                <?php  if(is_array($aagentlevels)) { foreach($aagentlevels as $level) { ?>
                    <?php  if(in_array( $level['id'] , $limitaagentlevels)) { ?>
                        <?php  echo $level['levelname']?>&nbsp;&nbsp;
                    <?php  } ?>
                <?php  } } ?>
            <?php  } ?>
