...
<ul class="nav nav-arrow-next nav-tabs" id="settingTab">
  ...
  <li ><a href="#tab_customnotice">客户消息设置</a></li> 
...
<form action="" method="post" class="form-horizontal form-validate" enctype="multipart/form-data">
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="tab_base"  ><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('sale/car/sendcar/base', TEMPLATE_INCLUDEPATH)) : (include template('sale/car/sendcar/base', TEMPLATE_INCLUDEPATH));?>
        </div>
        <div class="tab-pane" id="tab_customnotice"  ><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('sale/car/sendcar/customnotice', TEMPLATE_INCLUDEPATH)) : (include template('sale/car/sendcar/customnotice', TEMPLATE_INCLUDEPATH));?>
        </div>
        <div class="tab-pane" id="tab_setting"  ><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('sale/car/sendcar/templatenotice', TEMPLATE_INCLUDEPATH)) : (include template('sale/car/sendcar/templatenotice', TEMPLATE_INCLUDEPATH));?>
        </div>
    </div>
...
<script>
    ...
    function send(){
		...
		//tip.msgbox.err( '444!'); //跑到这。。
        $.ajax({
            url: "<?php  echo webUrl('sale/car/sendcar/fetch')?>",
            type:'post',
            dataType:'json',
            data:data,
    ...
    function sendmessage(){
        ...
        if(current>openids.length-1){
            if(failed.length>0){
                ...
                tip.msgbox.suc('发送成功 ' + succeed + ' 个用户!',"<?php  echo webUrl('sale/car/sendcar')?>" );
            }
            return;
        }
        ...
        $.ajax({
            url: "<?php  echo webUrl('sale/car/sendcar/sendmessage')?>",
            type:'post',
