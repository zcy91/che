...
/* 		$arr['template_id_short']="OPENTM200605630";
		$my_url="https://api.weixin.qq.com/cgi-bin/template/api_add_template?access_token={$token}";
		$arr=json_encode($arr);
		$res=ihttp_post($my_url,$arr); */
		$response = ihttp_request($post_url, $data);
		//file_put_contents(__dir__."/a.txt",json_encode($response));
		if(is_error($response)) {
			return error(-1, "访问公众平台接口失败, 错误: {$response['message']}");
		}
		$result = @json_decode($response['content'], true);
